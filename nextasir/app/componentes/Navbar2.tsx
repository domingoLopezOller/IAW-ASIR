    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.token) { // ¿Hay token? Si lo hay lo guardo en un localStorage
          ...
          setIsLoggedIn(true); //router.push("/bibliotecatic/admin/libros")
        } else {
          alert("Login failed: No se ha recibido el token")
          setIsLoggedIn(false); router.push("/biblioteca/libros")
        }
      } else {
  ...
  return (
    <nav style={{ backgroundColor: 'grey' }}>
      <div className="container mx-auto flex justify-between items-center">
        <Link href="/biblioteca/libros" className="text-xl font-bold">
          BibliotecaTIC
        </Link>
